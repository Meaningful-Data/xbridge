...
The validation engine accepts `.xbrl`, `.xml`, and `.zip` files. For `.zip`
files, the engine inspects the archive contents to determine the rule set:
...
- If the ZIP contains exactly one `.xbrl`/`.xml` file at the root level (and
  no `reports/report.json`), it is treated as an **XML-in-ZIP** and XML rules
  apply. The inner file is extracted for validation.
- If the ZIP contains multiple `.xbrl`/`.xml` files at root level, or no
  recognized content, validation raises an error.

### Implementation Status

The **Impl** column in each table indicates whether the rule is implemented in the
current codebase. Rules marked `Done` are registered in `registry.json`, have a
rule function in `src/xbridge/validation/rules/`, and are covered by tests.
...
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-001  | ERROR    | No  | --      | Done | The file MUST be well-formed XML.                                        |
| XML-002  | ERROR    | Yes | 1.4     | Done | The file MUST use UTF-8 encoding. (encodingNotUtf8)                      |

### 1.2 Root Element

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-003  | ERROR    | No  | --      | Done | The root element MUST be `xbrli:xbrl`.                                   |

### 1.3 Schema Reference (schemaRef)

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-010  | ERROR    | Yes | 1.5     | Done | Exactly one `link:schemaRef` element MUST be present. (multipleSchemaRefs) |
| XML-012  | ERROR    | Yes | 1.5     | Done | The `schemaRef` MUST resolve to a known entry point URL. (inappropriateSchemaRef) |

### 1.4 Filing Indicators

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-020  | ERROR    | Yes | 1.6     | Done | At least one `find:fIndicators` element MUST be present.                 |
| XML-021  | ERROR    | Yes | 1.6     | Done | At least one filing indicator (positive or negative) MUST exist. (missingFilingIndicators) |
| XML-024  | ERROR    | Yes | 1.6     | Done | Filing indicator values MUST match the label resources in the taxonomy. (invalidFilingIndicatorValue) |
| XML-025  | ERROR    | Yes | 1.6     | Done | No duplicate filing indicators for the same reporting unit. (duplicateFilingIndicator) |
| XML-026  | ERROR    | Yes | 1.6     | Done | The context referenced by filing indicators MUST NOT contain xbrli:segment or xbrli:scenario. (invalidContextForFilingIndicator) |

### 1.5 Context Structure

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-030  | ERROR    | Yes | 2.10    | Done | All `xbrli:period` date elements MUST be `xs:date` (no dateTime, no timezone). (periodWithTimeContent, periodWithTimezone) |
| XML-031  | ERROR    | Yes | 2.13    | Done | All periods MUST be instants (not durations). (nonInstantPeriodUsed)     |
| XML-032  | ERROR    | Yes | 2.13    | Done | All periods MUST refer to the same reference date. (multiplePeriodsUsed) |
| XML-033  | ERROR    | Yes | 2.9     | Done | All `xbrli:identifier` content and `@scheme` MUST be identical across contexts. (multipleIdentifiers) |
| XML-034  | ERROR    | Yes | 2.14    | Done | `xbrli:segment` elements MUST NOT be used. (segmentUsed)                |
| XML-035  | ERROR    | Yes | 2.15    | Done | `xbrli:scenario` children MUST only be `xbrldi:explicitMember` and/or `xbrldi:typedMember`. (scenarioContainsNonDimensionContent) |

### 1.6 Fact Structure

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-040  | ERROR    | Yes | 2.17    | Done | The `@decimals` attribute MUST be used (not `@precision`). (precisionUsed) |
| XML-041  | ERROR    | No  | --      | Done | The `@decimals` value MUST be a valid integer, "INF", or absent for non-numeric. |
| XML-042  | ERROR    | Yes | 2.19    | Done | `@xsi:nil` MUST NOT be used on facts. (nilUsed)                         |
| XML-043  | ERROR    | Yes | 2.19    | Done | String-type facts MUST NOT be empty. (emptyUsed)                        |

### 1.7 Unit Structure

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-050  | ERROR    | Yes | 2.23    | Done | `xbrli:unit` children MUST refer to the XBRL International Unit Type Registry (UTR). (nonUtrUnit) |

### 1.8 Document-level Checks

| Rule ID  | Severity  | EBA | EBA ref | Impl | Description                                                             |
|----------|-----------|-----|---------|------|-------------------------------------------------------------------------|
| XML-060  | ERROR     | Yes | 2.1     | Done | `@xml:base` MUST NOT appear anywhere in the document. (xmlBaseUsed)     |
| XML-061  | ERROR     | Yes | 2.4     | Done | `link:linkbaseRef` MUST NOT be used. (linkbaseRefUsed)                  |
| XML-062  | ERROR     | Yes | 2.11    | Done | `xbrli:forever` MUST NOT be used. (foreverUsed)                        |
| XML-063  | ERROR     | Yes | 1.14    | Done | `@xsd:schemaLocation` / `@xsd:noNamespaceSchemaLocation` MUST NOT be used. (schemaLocationAttributeUsed) |
| XML-064  | ERROR     | Yes | 1.15    | Done | `xi:include` MUST NOT be used. (xIncludeUsed)                          |
| XML-065  | WARNING   | Yes | 1.13    | Done | The XML standalone declaration SHOULD NOT be used. (standaloneDocumentDeclarationUsed) |
| XML-066  | WARNING   | Yes | 2.7     | Done | Unused contexts SHOULD NOT be present. (unusedContext)                  |
| XML-067  | WARNING   | Yes | 2.7     | Done | Duplicate contexts SHOULD NOT be present. (duplicateContext)            |
| XML-068  | WARNING   | Yes | 2.22    | Done | Unused units SHOULD NOT be present. (unusedUnit)                       |
| XML-069  | WARNING   | Yes | 2.21    | Done | Duplicate units SHOULD NOT be present. (duplicateUnit)                 |

### 1.9 Taxonomy Conformance

| Rule ID  | Severity | EBA | EBA ref | Impl | Description                                                              |
|----------|----------|-----|---------|------|--------------------------------------------------------------------------|
| XML-070  | ERROR    | No  | --      | Done | All fact element names (concepts) MUST be defined in the taxonomy for the referenced entry point. |
| XML-071  | ERROR    | No  | --      | Done | All explicit dimension QNames used in contexts MUST be defined as dimensions in the taxonomy. |
| XML-072  | ERROR    | No  | --      | Done | All dimension member values MUST be valid members for their respective dimension as defined in the taxonomy. |

### 1.10 Entity Identification

| Rule ID        | Severity | EBA | EBA ref | Impl | Description                                                      |
|----------------|----------|-----|---------|------|------------------------------------------------------------------|
| EBA-ENTITY-001 | ERROR    | Yes | 2.8     | Done | The entity identifier `@scheme` MUST be one of the accepted schemes (`http://standards.iso.org/iso/17442` for LEI, `https://eurofiling.info/eu/rs` for qualified identifiers). (inappropriateScheme) |
| EBA-ENTITY-002 | ERROR    | Yes | 2.8     | Done | The entity identifier value MUST follow the reporting subject conventions (LEI format, consolidation suffix, etc.). (unacceptableIdentifier) |

### 1.11 Decimals Accuracy

| Rule ID     | Severity | EBA | EBA ref | Impl | Description                                                           |
|-------------|----------|-----|---------|------|-----------------------------------------------------------------------|
| EBA-DEC-001 | ERROR    | Yes | 2.18    | Done | Monetary facts: `@decimals` MUST be >= -4 (from 01/04/2025; >= -6 for FP, ESG, Pillar3, REM_DBM modules). |
| EBA-DEC-002 | ERROR    | Yes | 2.18    | Done | Percentage facts: `@decimals` MUST be >= 4.                          |
| EBA-DEC-003 | ERROR    | Yes | 2.18    | Done | Integer facts: `@decimals` MUST be 0.                                |
| EBA-DEC-004 | WARNING  | Yes | 2.18    | Done | Decimals SHOULD be a realistic indication of accuracy (not excessively high). |

### 1.12 Currency Rules

| Rule ID     | Severity | EBA | EBA ref | Impl | Description                                                           |
|-------------|----------|-----|---------|------|-----------------------------------------------------------------------|
| EBA-CUR-001 | ERROR    | Yes | 3.1     | Done | All monetary facts without CCA dimension `eba_CA:x1` (or qAEA `eba_qCA:qx2000`) MUST use a single "reporting currency". (multipleReportingCurrencies) |
| EBA-CUR-002 | ERROR    | Yes | 3.1     | Done | Facts with CCA `eba_CA:x1` (or qAEA `eba_qCA:qx2000`) MUST be expressed in their currency of denomination. (currencyOfDenomination) |
| EBA-CUR-003 | ERROR    | Yes | 3.1     | Done | For facts with CUS or CUA dimension, the unit currency MUST be consistent with the dimension value. (inconsistentCurrencyUnitAndDimension) |

### 1.13 Non-monetary Numeric Values

| Rule ID      | Severity | EBA | EBA ref | Impl | Description                                                          |
|--------------|----------|-----|---------|------|----------------------------------------------------------------------|
| EBA-UNIT-001 | ERROR    | Yes | 3.2     | Done | Non-monetary numeric values MUST use the "pure" unit. (pureUnitNotUsedForMonetaryValue) |
| EBA-UNIT-002 | WARNING  | Yes | 3.2     | Done | Rates, percentages and ratios SHOULD use decimal notation (e.g. 0.0931 not 9.31). A warning is raised when the absolute value exceeds 50, which strongly suggests percentage notation was used instead of decimal fractions. (useDecimalFractions) |

### 1.14 Additional Checks

| Rule ID     | Severity | EBA | EBA ref | Impl | Description                                                           |
|-------------|----------|-----|---------|------|-----------------------------------------------------------------------|
| EBA-2.5     | WARNING  | Yes | 2.5     | Done | XML comments are ignored; data SHOULD only appear in contexts, units, and facts. (xmlCommentPresent) |
| EBA-2.16.1  | ERROR    | Yes | 2.16    | Done | No multi-unit fact sets; facts MUST NOT differ only by unit. (factsDifferingOnlyByUnit) |
| EBA-2.24    | ERROR    | Yes | 2.24    | Done | Monetary units MUST be basic ISO 4217 without scaling. (nonBasicMonetaryUnit) |
| EBA-2.25    | WARNING  | Yes | 2.25    | Done | Footnotes are ignored. `link:footnoteLink` SHOULD NOT be present. (footnotePresent) |

### 1.15 Guidance

| Rule ID        | Severity | EBA | EBA ref | Impl | Description                                                       |
|----------------|----------|-----|---------|------|-------------------------------------------------------------------|
| EBA-GUIDE-001  | WARNING  | Yes | 3.4     | Done | Unused namespace prefixes SHOULD NOT be declared. (unusedNamespacePrefix) |
| EBA-GUIDE-002  | WARNING  | Yes | 3.4     | Done | Namespace prefixes SHOULD mirror canonical prefixes. (notRecommendedNamespacePrefix) |
| EBA-GUIDE-003  | WARNING  | Yes | 3.4     | Done | Unused `@id` on facts SHOULD NOT be present. (unusedFactId)      |
| EBA-GUIDE-004  | WARNING  | Yes | 3.4     | Done | String values SHOULD be as short as possible. (excessiveStringLength) |
| EBA-GUIDE-005  | WARNING  | Yes | 3.4     | Done | Namespace declarations SHOULD be on the document element only. (unexpectedNamespaceDeclarations) |
| EBA-GUIDE-006  | WARNING  | Yes | 3.4     | Done | Avoid multiple prefix declarations for the same namespace. (multiplePrefixForNamespace) |
| EBA-GUIDE-007  | WARNING  | Yes | 3.4     | Done | String facts and domain values SHOULD NOT start/end with whitespace. (leadingOrTrailingSpacesInText) |
...
> **Note:** CSV rules are not yet implemented. The sections below define the
> planned rule set.
...
**Scope:** File naming conventions for EBA report submissions. These rules
validate the file name structure for both bare `.xbrl`/`.xml` files and `.zip`
...
| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-001  | ERROR    | Yes | File naming   | Done | The file name MUST follow the pattern `ReportSubject_Country_FrameworkCodeModuleVersion_Module_ReferenceDate_CreationTimestamp` (exactly six underscore-separated components plus extension). |

### 3.2 Report Subject

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-010  | ERROR    | Yes | File naming   | Done | For modules with `_con`/`_ind` in the name (or before reference date 31/12/2022): the ReportSubject MUST be the LEI (individual/consolidated) or LEI + `.CRDLIQSUBGRP` (liquidity subgroup). |
| EBA-NAME-011  | ERROR    | Yes | File naming   | Done | For modules without `_con`/`_ind` (from reference date 31/12/2022): individual reports MUST use LEI + `.IND`, consolidated MUST use LEI + `.CON`, liquidity subgroup MUST use LEI + `.CRDLIQSUBGRP`. |
| EBA-NAME-012  | ERROR    | Yes | File naming   | Done | For country-level aggregate reports: the ReportSubject MUST be the ISO country code (two capital letters) + `000` + the aggregation type suffix (`.MEMSTAAGGALL`, `.MEMSTAAGGCRDCREINS`, `.MEMSTAAGGINVFIR`). |
| EBA-NAME-013  | ERROR    | Yes | File naming   | Done | For authority-level aggregate reports: the ReportSubject MUST be the authority code (agreed with EBA) + `.AUTALL` (e.g. for MREL Decisions). |
| EBA-NAME-014  | ERROR    | Yes | File naming   | Done | For MICA reports: the ReportSubject MUST be IssuerID-TokenID + `.IND`. |

### 3.3 Country

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-020  | ERROR    | Yes | File naming   | Done | The Country component MUST be a valid ISO 3166-1 alpha-2 country code in uppercase (e.g. `DE`, `IT`, `FR`). |

### 3.4 Framework Code and Module Version

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-030  | ERROR    | Yes | File naming   | Done | The FrameworkCodeModuleVersion MUST be the DPM/XBRL framework code in uppercase followed by the module version as a 6-digit number `XXYYZZ` (e.g. `COREP020001` for COREP taxonomy 2.0.1: XX=02, YY=00, ZZ=01). |

### 3.5 Module

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-040  | ERROR    | Yes | File naming   | Done | The Module component MUST be the taxonomy module name in uppercase without underscores (e.g. `corep_lcr_con` becomes `COREPLCRCON`). For liquidity subgroup reports, it MUST be the relevant consolidated module name in uppercase (e.g. `COREPLCRDACON`). |

### 3.6 Reference Date

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-050  | ERROR    | Yes | File naming   | Done | The ReferenceDate component MUST be in `YYYY-MM-DD` format (e.g. `2025-12-31`). |

### 3.7 Creation Timestamp

| Rule ID       | Severity | EBA | EBA ref       | Impl | Description                                                                    |
|---------------|----------|-----|---------------|------|--------------------------------------------------------------------------------|
| EBA-NAME-060  | ERROR    | Yes | File naming   | Done | The CreationTimestamp component MUST be in `YYYYMMDDhhmmssfff` format (e.g. `20250327095525486`). |
